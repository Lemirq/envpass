...
export default function DashboardPage() {
  ...
  const rooms = useQuery(api.rooms.listMyRooms, userId ? { userId } : "skip");
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      {/* Header */}
      <header className="h-16 border-b border-(--border-color) flex items-center justify-between px-6 bg-[rgba(5,5,5,0.8)] backdrop-blur-sm z-10 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/"
            className="font-bold text-lg tracking-tight no-underline text-[var(--text-primary)]"
          >
            envpass
          </Link>
          <div
            className="text-[0.7rem] text-[var(--text-secondary)] mt-0.5 uppercase"
      ...
      <main className="flex-1 overflow-y-auto p-8">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-end justify-between mb-8">
            <div>
              <div
                className="text-[0.75rem] uppercase tracking-[0.15em] text-[var(--text-secondary)] mb-2"
                style={{ fontFamily: "var(--font-pixel)" }}
              >
                Dashboard
              </div>
          ...
          {isLoading ? (
            ...
            <div className="border border-[var(--border-color)] rounded-xl bg-[var(--bg-card)] overflow-hidden shadow-[0_4px_24px_rgba(0,0,0,0.3)]">
              ...
              <div className="grid grid-cols-[2fr_1fr_1fr_1fr] px-5 py-3 border-b border-[var(--border-color)] bg-[rgba(255,255,255,0.02)]">
                <div className="text-[0.65rem] uppercase tracking-[0.15em] text-[var(--text-secondary)] font-semibold">
                  Room
                </div>
                <div className="text-[0.65rem] uppercase tracking-[0.15em] text-[var(--text-secondary)] font-semibold">
                  Code
